    ...
    <div className="flex flex-col h-full bg-brand-surface/70 backdrop-blur-md border border-white/10 rounded-xl shadow-lg">
      ...
      <div className="border-t border-white/10 p-4">
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          {/* Top Row: Trending Searches and Provider Selector */}
          <div className="flex items-center justify-between gap-3">
            <button
              type="button"
              onClick={() => setShowPresets((prev) => !prev)}
              className="inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 hover:border-brand-primary/40 text-sm font-semibold text-brand-text-light transition"
            >
          ...
          <div className="relative w-full">
            <input
              ...
              className="w-full bg-gray-800/60 border border-white/15 rounded-xl py-3 pl-4 pr-14 text-brand-text-light focus:outline-none focus:ring-2 focus:ring-brand-primary transition-all duration-300 placeholder:text-brand-text-dark"
              disabled={isLoading}
            />
            <button
              type="submit"
